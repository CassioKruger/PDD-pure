...
Group {
  DefineGroup[
    ...
    //Rotor2
    Rotor2_Inds, Rotor2C, Rotor2CC, Rotor2_Air, Rotor2_Airgap, Rotor2_Magnets, Rotor2_Bars,
    Rotor2_IndsP, Rotor2_IndsN, Rotor2_Al, Rotor2_Cu, Rotor2_Fe, Rotor2_Top_Bnd_MBaux, Rotor2_Bottom_Bnd_MBaux,
    Rotor2_Top_Bnd_MB, Rotor2_Bottom_Bnd_MB,
...
DefineConstant[
  ...
  mur_fe = 1.0e3,     // magnetic permeability of iron
...
Function{
  DefineFunction[
    ...
    Rotor2Position, Rotor2Position_deg,
...
Group {
  ...
  //   DomainKin2  : region number for mechanical equation for MB2
  //   DomainDummy : region number for postpro with functions
  //   DomainDummy2 : region number for postpro with functions for MB2
  ...
  Rotor2  = Region[{ Rotor2C, Rotor2CC }];

  Rotor_Moving = Region[{ Rotor, Rotor_Air, Rotor_Airgap, Rotor_Inds, Rotor_Bnd_MBaux} ] ;
  Rotor2_Moving = Region[{ Rotor2, Rotor2_Air, Rotor2_Airgap, Rotor2_Inds, Rotor2_Top_Bnd_MBaux, Rotor2_Bottom_Bnd_MBaux} ] ;
...
Function {
  mu0 = 4.e-7 * Pi ;
  nu0 = 1. / mu0 ;
  nu [Region[{Air, Inds, Stator_Cu, Rotor_Magnets, Stator_Magnets }]]=nu0;

  If(!Flag_NL)
    nu [Region[{Stator_Fe, Rotor_Fe, Rotor2_Fe}]]  = 1 / (mur_fe * mu0) ;
    dhdb_NL [] = 0;
  EndIf
  If(Flag_NL)
    If(Flag_NL_law_Type==0)
      nu [Region[{Stator_Fe, Rotor_Fe, Rotor2_Fe}]] = nu_1a[$1] ;
      dhdb_NL [ DomainNL ] = dhdb_1a_NL[$1];
    EndIf
    If(Flag_NL_law_Type==1)
      nu [Region[{Stator_Fe, Rotor_Fe, Rotor2_Fe}]] = nu_1[$1] ;
      dhdb_NL [ DomainNL ] = dhdb_1_NL[$1];
    EndIf
    If(Flag_NL_law_Type==2)
      nu [Region[{Stator_Fe, Rotor_Fe, Rotor2_Fe}]] = nu_3kWa[$1] ;
      dhdb_NL [ DomainNL ] = dhdb_3kWa_NL[$1];
    EndIf
    If(Flag_NL_law_Type==3)
      nu [Region[{Stator_Fe, Rotor_Fe, Rotor2_Fe}]] = nu_3kW[$1] ;
      dhdb_NL [ DomainNL ] = dhdb_3kW_NL[$1];
    EndIf
  EndIf

  sigma[Region[{Rotor_Fe, Rotor2_Fe, Stator_Fe}]] = sigma_fe ;
  sigma[Region[{Rotor_Al, Rotor2_Al, Stator_Al}]] = sigma_al ;
  sigma[Region[{Rotor_Cu, Rotor2_Cu, Stator_Cu}]] = sigma_cu ;
  sigma[Inds] = sigma_cu ;

  rho[] = 1/sigma[] ;

  Rb[] = Factor_R_3DEffects*AxialLength*FillFactor_Winding*NbWires[]^2/SurfCoil[]/sigma[] ;
  Resistance[Region[{Stator_Inds, Rotor_Inds, Rotor2_Inds}]] = Rb[] ;

  Idir[Region[{Stator_IndsP, Rotor_IndsP, Rotor2_IndsP}]] =  1 ;
  Idir[Region[{Stator_IndsN, Rotor_IndsN, Rotor2_IndsN}]] = -1 ;
  ...
  { Name Hcurl_a_2D ; Type Form1P ;
    BasisFunction {
      { Name se1 ; NameOfCoef ae1 ; Function BF_PerpendicularEdge ;
        Support Region[{ Domain, Rotor_Bnd_MBaux, Rotor2_Top_Bnd_MBaux,Rotor2_Bottom_Bnd_MBaux }] ; Entity NodesOf [ All ] ; }
   }
    Constraint {
...
PostProcessing {
  ...
  { Name MagStaDyn_a_2D ; NameOfFormulation MagStaDyn_a_2D ;
   ...
   // Torque computation via Virtual Works
     ...
     { Name IB2  ;
       Value { Term { Type Global; [ IB[] ] ; In DomainDummy2 ; } } }
     { Name IC2  ;
       Value { Term { Type Global; [ IC[] ] ; In DomainDummy2 ; } } }
     { Name Flux_d2  ;
       Value { Term { Type Global; [ CompX[Flux_dq0[]] ] ; In DomainDummy2 ; } } }
     { Name Flux_q2  ;
...
po_mecT = StrCat[po_mec,"2Torque [Nm]/"];
